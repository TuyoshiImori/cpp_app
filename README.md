...
2. **画像処理と回答検出**
   ...
   - 円検出を行い、設問ごとに画像を切り取り。
   - チェックボックス検出・自由記述OCR・個人情報フィールド検出を実施。
   - AI（FoundationModels）によるテキスト補正を実行。
...
4. **エクスポートとブラウザ連携**
   - プレビュー画面右上の「エクスポート」ボタンから `ExportView` を開く。
   - 「ZIP を作成する」をタップすると設問画像・回答結果・manifest.json を格納した ZIP を生成。
   - **Wi-Fi 転送**: `LocalHttpServer`（Network.framework）が起動し、同一 Wi-Fi 上のブラウザに URL を表示してダウンロードリンクを提供する。
   - **AirDrop・共有**: `ActivityView` 経由で AirDrop やファイルアプリに転送できる。
   - ブラウザ側では Web アプリの「回答を受け取る」ページ（`/receive`）で ZIP を読み込み、「スキャン結果」ページ（`/results`）で回答の確認・編集・CSV 出力が可能。

### ZIP の構成

```
survey_YYYYMMDD_HHMMSS.zip
├── manifest.json          # アンケートメタ情報（タイトル、設問タイプ一覧）
├── page_001/
│   ├── answers.json       # 各設問の回答・信頼度
│   ├── q_001.jpg          # 設問 1 の切り取り画像
│   ├── q_002.jpg
│   └── ...
├── page_002/
│   └── ...
